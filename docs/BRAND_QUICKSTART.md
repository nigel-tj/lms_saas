...
| `app_name` is set to the corporate name in `hooks.py` | `app_title` / `app_name` in `hooks.py` is the source of Frappe's `app_name` fallback. If it leaks the operator's name, a fresh install picks it up. | The R23 test (`TestR23AppTitleIsVendorNeutral`) pins `app_title == "LMS"`. Don't change this. |
