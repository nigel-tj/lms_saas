...
MONEY_DOCTYPES = ("Loan", "Loan Disbursement", "Loan Repayment", "LMS Investor Transaction")
...
def write_audit_event(event_type, reference_doctype, reference_name, amount=None, company=None, details=None):
    """Append an immutable audit event. Never raises into the caller's flow."""
    try:
        frappe.get_doc(
            {
    ...
    except Exception:  # noqa: BLE001 - auditing must not break business flow
