"""Desk navigation payload for admins (boot session + route → workspace mapping)."""
...
from lms_saas.install import ADMIN_ROLES, LMS_NAV_SPEC, _resolve_workspace_spec
...
def get_lms_desk_nav(user: str | None = None) -> dict:
	...
	if not user_roles.intersection(DESK_ADMIN_ROLES):
		return {"enabled": False}
