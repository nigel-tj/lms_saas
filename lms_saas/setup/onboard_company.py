...
def _detect_plan(company: str, email_id: str | None = None) -> dict[str, Any]:
    from lms_saas.install import ALL_LMS_ROLES, LMS_NAV_SPEC
    ...
    missing_roles = [role for role in ALL_LMS_ROLES if not frappe.db.exists("Role", role)]
...
def _apply_core_setup(company: str) -> None:
    ...
    install_mod._setup_lms_roles()
    install_mod._ensure_lms_page_permissions()
    ...
    install_mod._setup_module_lockdown()
