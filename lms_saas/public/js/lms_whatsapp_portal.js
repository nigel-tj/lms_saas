...
lms_whatsapp._renderTemplatesList = function () {
	...
	lms_portal.safeCall({
		...
		callback: function (r) {
			var templates = (r && r.message && r.message.templates) || [];
			if (!templates.length) {
				el.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("file-text") + '<h3>No templates</h3><p>No WhatsApp templates found.</p></div></div>';
				return;
			}
...
lms_whatsapp._loadLog = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			...
			if (!logs.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("scroll") + '<h3>No messages</h3><p>No WhatsApp messages logged.</p></div></div>';
				return;
			}
			var html = '<div class="lms-panel"><div class="lms-data-table__wrap"><table class="lms-data-table">';
