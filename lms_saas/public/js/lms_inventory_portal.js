...
lms_inventory._loadAssets = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			...
			if (!assets.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("package") + '<h3>No assets</h3><p>No assets found for your branch.</p></div></div>';
				return;
			}
...
lms_inventory._loadStock = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			...
			if (!items.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("clipboard") + '<h3>No stock items</h3><p>No stock items found.</p></div></div>';
				return;
			}
			var html = '<div class="lms-panel"><div class="lms-data-table__wrap"><table class="lms-data-table">';
...
lms_inventory._loadLowStock = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			var items = (r && r.message && r.message.items) || [];
			if (!items.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("check") + '<h3>All stocked</h3><p>No items below reorder level.</p></div></div>';
				return;
			}
			var html = '<div class="lms-panel"><div class="lms-data-table__wrap"><table class="lms-data-table">';
