...
lms_savings.init = function () {
	...
	var tabs = [
		{ id: "accounts", label: "My Savings", icon: "bank" },
		{ id: "goals", label: "Goals", icon: "target" },
		{ id: "transactions", label: "Transactions", icon: "clipboard" },
	];
...
lms_savings._loadAccounts = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			...
			if (!accounts.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("bank") + '<h3>No savings accounts</h3><p>You have no savings accounts yet.</p></div></div>';
				return;
			}
...
lms_savings._loadGoals = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			...
			if (!goals.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("target") + '<h3>No savings goals</h3><p>No savings goals set yet.</p></div></div>';
				return;
			}
...
lms_savings._loadTransactions = function (content) {
	lms_portal.safeCall({
		...
		callback: function (r) {
			var accounts = (r && r.message && r.message.accounts) || [];
			if (!accounts.length) {
				content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("clipboard") + '<h3>No accounts</h3><p>No savings accounts to show transactions for.</p></div></div>';
				return;
			}
			...
			lms_portal.safeCall({
				...
				callback: function (r2) {
					...
					if (!txns.length) {
						content.innerHTML = '<div class="lms-panel"><div class="lms-empty">' + lms_icons.empty("clipboard") + '<h3>No transactions</h3><p>No transactions yet.</p></div></div>';
						return;
					}
